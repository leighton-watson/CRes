function P = pressurePerturbation_slope(input, style, M, slope_angle)
...
% style = 'baffled piston' or 'monopole' description of sound radiation
    ...
    if strcmp(style,'baffled piston')
        ...
        ang = 2*slope_angle*(2*pi/360)/pi;
        
        p = 1/(2+ang)*1i.*omega*rho.*u.*a.^2 .* ((exp(-1i*k*r))/r) .* ...
    ...
    elseif strcmp(style,'monopole')
        ...
        deltaP = rho*vDot*ventArea / ((2+ang)*pi*r); % pressure perturbation
        P = deltaP;
