function output = resonance1d_Temp(geometry, depth, freq, Nf, style, order, Tmin, Tmax)
...
% output.P = far-field pressure perturbation transfer function
    ...
    % truncate M to desired depth
    %Tmin = 0;
    %Tmax = 200;
    T_profile = (Tmax-Tmin)/depth*z + Tmin;
    M = problemParameters_Temp(T_profile);
    ...
    Z0 = M.rhoC(1)*M.cC(1)/S(1); %characteristc impedance of pipe
    ZN = M.rhoC(end)*M.cC(end)/S(end);
    ...
    output.P = pressurePerturbation(output, style, M); % far-field pressure perturbation transfer function
    if output.f(1) == 0
